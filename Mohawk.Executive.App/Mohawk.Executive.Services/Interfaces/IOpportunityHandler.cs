...
namespace Mohawk.Executive.Services.Interfaces
{
    ...
    {
        OpportunityModel AddOpportunity(Guid contactId, string subject,string description, string estimatedValue, int priority, string userId);
        bool RemoveOpportunity(Guid opportunityId);
        OpportunityModel UpdateOpportunity(Guid opportunityId, string newSubject,string newDescription, string newValue, int? newPriority = null);
